...
void solve()
{
    ...
    vll v(10);
    forA(i,1,10,1)cin>>v[i];

    ll minElement = *min_element(v.begin()+1,v.end());
    ll minIndex = -1;
    for(ll i=9; i>0; i--)if(v[i]==minElement){minIndex = i; break;}
    ll maxDigitsPossible = n/minElement;
    ll total = maxDigitsPossible*minElement;

    string ans = "";
    forA(i,0,maxDigitsPossible,1)ans+=('0'+minIndex);
    ll p = 0;

    for(ll i=9; i>0; i--)
    {
        //replace a num with i;
        ll diff = v[i]-minElement;

        if(diff<=0 || p>=ans.length())
            break;

        ll left = n-total;
        ll toChange = left/diff;
